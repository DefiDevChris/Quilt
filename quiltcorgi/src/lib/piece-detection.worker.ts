...
function getMinAreaRatioForPieceScale(pieceScale: QuiltDetectionConfig['pieceScale']): number {
  switch (pieceScale) {
    case 'tiny':
      return PHOTO_PATTERN_PIECE_MIN_AREA_RATIO * 0.4;
    case 'large':
      return PHOTO_PATTERN_PIECE_MIN_AREA_RATIO * 2.5;
    case 'standard':
    default:
...
/**
 * Polygon approximation with curved piecing support.
 * Uses smaller epsilon for curved piecing to preserve more points.
 */
