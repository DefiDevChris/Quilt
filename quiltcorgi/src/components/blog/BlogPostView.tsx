...
export function BlogPostView({ post, relatedPosts = [] }: BlogPostViewProps) {
  return (
    <article>
      ...
      <Link
        href="/blog"
        className="text-sm text-secondary hover:text-on-surface transition-colors mb-6 inline-block"
      >
        &larr; Back to Blog
      </Link>
      ...
      <header className="mb-8">
        {/* Category */}
        <span className="inline-block text-xs font-medium text-primary bg-primary-container/30 px-2.5 py-1 rounded-full mb-3">
          {post.category}
        </span>
        ...
        <div className="flex items-center gap-3 text-sm text-secondary">
          ...
          <span className="font-medium text-on-surface">{post.author.name}</span>
      ...
      {/* Author Bio Card */}
      <div className="border-t border-outline-variant/20 pt-8 mb-12">
        <div className="flex items-start gap-4 bg-surface-container rounded-xl p-6">
          {post.author.avatarUrl ? (
            <Image
              src={post.author.avatarUrl}
              alt={post.author.name}
          ...
          <div>
            ...
            {post.author.bio && (
              <p className="text-sm text-secondary mt-1">{post.author.bio}</p>
            )}
            ...
            {relatedPosts.map((related) => (
              ...
              >
                {related.featuredImageUrl && (
                  <div className="aspect-[16/9] overflow-hidden rounded-md mb-3 bg-surface-container-high">
                    <Image
                ...
                <h3 className="text-sm font-medium text-on-surface group-hover:text-primary transition-colors line-clamp-2">
                  {related.title}
                </h3>
