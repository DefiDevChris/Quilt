...
import { TrendingContent } from '@/components/social/TrendingContent';
...
export default async function SocialThreadsPage({ searchParams }: SocialThreadsPageProps) {
  ...
  // Validate section
  const validSections: SectionId[] = ['feed', 'blog', 'featured', 'trending'];
  ...
  return (
    <SocialLayout activeSection={activeSection}>
      <Suspense fallback={<div className="animate-pulse space-y-6">
        ...
        {activeSection === 'trending' && <TrendingContent />}
        {activeSection === 'blog' && (
