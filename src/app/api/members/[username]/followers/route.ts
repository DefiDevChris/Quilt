...
export async function GET(
  ...
  const rl = await checkRateLimit(`followers:${ip}`, API_RATE_LIMITS.like);
  ...
  try {
    const [followerRows, [totalRow]] = await Promise.all([
      db
        .select({
    ...
    return errorResponse('Failed to load followers.', 'INTERNAL_ERROR', 500);
  }
}
