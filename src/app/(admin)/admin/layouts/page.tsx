    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-dim mt-1">Manage quilt layout templates available in the studio</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="bg-[var(--color-bg)]">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-dim hidden sm:table-cell">Category</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-dim text-center">Published</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-dim hidden md:table-cell">Created</th>
              ...
              {layouts.map((layout) => (
                <tr key={layout.id} className="hover:bg-[var(--color-bg)]/60 transition-colors duration-150">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {layout.isDefault && (
                          <span className="text-xs px-1.5 py-0.5 rounded-lg" style={{ backgroundColor: withAlpha(COLORS.primary, 0.1), color: COLORS.textDim }}>System</span>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 hidden sm:table-cell"><span className="text-sm text-dim">{layout.category}</span></td>
                  <td className="px-4 py-3 text-center">
                    ...
                    >
                      <span className={`inline-block h-4 w-4 transform rounded-full bg-[var(--color-surface)] transition-colors duration-150 ${layout.isPublished ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className="text-sm text-dim">{new Date(layout.createdAt).toLocaleDateString()}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setEditingLayout(layout)} className="text-sm font-medium text-primary hover:text-primary-hover transition-colors duration-150">Edit</button>
                      <button
      ...
      {showCreateModal && <LayoutFormModal onClose={() => setShowCreateModal(false)} onSaved={handleSaved} />}
      {editingLayout && <LayoutFormModal layout={editingLayout} onClose={() => setEditingLayout(null)} onSaved={handleSaved} />}
    </div>
  );
}

function LayoutFormModal({ layout, onClose, onSaved }: { layout?: LayoutTemplate; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <AdminModal title={isEditing ? 'Edit Layout' : 'Create New Layout'} error={error} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="layout-name" className="text-sm font-medium text-default">Name *</label>
            <input id="layout-name" required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full px-3 py-2 border border-default rounded-lg bg-[var(--color-surface)]" placeholder="e.g. Classic Nine-Patch" />
          </div>
          <div>
            <label htmlFor="layout-category" className="text-sm font-medium text-default">Category</label>
            <select id="layout-category" name="category" value={formData.category} onChange={handleChange} className="w-full px-3 py-2 border border-default rounded-lg bg-[var(--color-surface)]">
        ...
        <div>
          <label htmlFor="layout-template-data" className="text-sm font-medium text-default">Template Data (JSON) *</label>
          <textarea id="layout-template-data" required name="templateDataJson" value={formData.templateDataJson} onChange={handleChange} rows={6} className="w-full px-3 py-2 border border-default rounded-lg bg-[var(--color-surface)] font-mono text-xs" placeholder='{"shapes": [...]}' />
        </div>
        <div>
          <label htmlFor="layout-thumbnail-svg" className="text-sm font-medium text-default">Thumbnail SVG</label>
          <textarea id="layout-thumbnail-svg" name="thumbnailSvg" value={formData.thumbnailSvg} onChange={handleChange} rows={3} className="w-full px-3 py-2 border border-default rounded-lg bg-[var(--color-surface)] font-mono text-xs" placeholder='<svg viewBox="0 0 100 100">...</svg>' />
        </div>
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={formData.isDefault} onChange={(e) => setFormData((prev) => ({ ...prev, isDefault: e.target.checked }))} className="rounded-lg border-default" />
            <span className="text-sm text-default">System Default</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={formData.isPublished} onChange={(e) => setFormData((prev) => ({ ...prev, isPublished: e.target.checked }))} className="rounded-lg border-default" />
