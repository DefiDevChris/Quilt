...
export function DuplicateOptionsPopup() {
  ...
  const dialogRef = useFocusTrap<HTMLDivElement>(show, () => setShow(false));
  ...
  const duplicateToCurrent = async () => {
    ...
    const clonePromises = objects.map((obj) => (obj as { clone: () => Promise<unknown> }).clone());
  ...
  const duplicateToNew = async () => {
    ...
    if (activeWorktable) {
      await (canvas as unknown as { loadFromJSON: (data: unknown) => Promise<void> }).loadFromJSON(
        activeWorktable.canvasData
      );
