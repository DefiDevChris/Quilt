...
import { clearCanvasFabrics } from '@/lib/clear-canvas-fabrics';
 ...
 */
/**
 * SaveAsTemplateButton — opens the Save as Template modal.
 *
 * Per the design studio spec, when a user has a complete design they can
 * save it as a reusable template. The button is gated on `hasContent` so
 * empty canvases don't surface the action. Available in all three modes
 * (template, layout, freeform) — your reusable templates can come from
 * any source.
 */
      ...
      <TooltipHint
        ...
        description={
          hasContent
            ...
            : 'Add some content to your design first'
        }
      >
        <button
          type="button"
          ...
          disabled={!hasContent}
          className="rounded-full border border-[var(--color-border)]/40 bg-[var(--color-surface)] px-3 py-1 text-[12px] leading-[18px] font-medium text-[var(--color-text)]/75 hover:text-[var(--color-text)] hover:bg-[var(--color-bg)] transition-colors disabled:cursor-not-allowed disabled:opacity-40"
          aria-label="Save as Template"
        >
          Save as Template
        </button>
      </TooltipHint>
      <SaveAsTemplateModal isOpen={isOpen} onClose={() => setIsOpen(false)} />
    </>
  );
}

/**
 * ClearFabricsButton — visible only in Template mode.
 *
 * Per the design studio spec, templates ship with all pieces and fabrics
 * pre-applied. This button strips every fabric fill from the canvas and
 * leaves just the bare shapes — letting the user start their own fabric
 * exploration from the template's structure.
 */
function ClearFabricsButton() {
  const projectMode = useProjectStore((s) => s.mode);
  const { getCanvas } = useCanvasContext();
  const { toast } = useToast();

  if (projectMode !== 'template') return null;

  const handleClick = () => {
    const canvas = getCanvas();
    if (!canvas) return;
    clearCanvasFabrics(canvas);
    useProjectStore.getState().setDirty(true);
    toast({
      type: 'success',
      title: 'Fabrics cleared',
      description: 'The template canvas is now empty shapes.',
    });
  };
  ...
  return (
    <TooltipHint
      ...
      description="Remove every fabric from this template, leaving the bare shapes."
    >
      <button
        type="button"
        onClick={handleClick}
        className="rounded-full border border-[var(--color-border)]/40 bg-[var(--color-surface)] px-3 py-1 text-[12px] leading-[18px] font-medium text-[var(--color-text)]/75 hover:text-[var(--color-text)] hover:bg-[var(--color-bg)] transition-colors"
