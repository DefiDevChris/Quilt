        ...
        <div className="relative aspect-[4/3] cursor-pointer overflow-hidden"
          ...
          <div className="absolute top-3 right-3">
            <button onClick={(e) => { e.stopPropagation(); handleLike(); }}
              className={cn('p-2.5 rounded-full', liked ? 'bg-[#ff8d49] text-white' : 'bg-[var(--color-surface)] text-[var(--color-text-dim)]')}>
              <Heart className={cn('h-4 w-4', liked && 'fill-current')} />
            </button>
          </div>
          {post.isFeatured && (
            <div className="absolute top-3 left-3">
              <span className="px-3 py-1 bg-[#ff8d49] text-white text-xs font-medium rounded-full">Featured</span>
            </div>
          )}
        </div>
        <div className="p-4">
          <div className="flex items-center gap-2 mb-2">
            <Avatar className="h-8 w-8 cursor-pointer border-2 border-[var(--color-border)]" onClick={() => onUserClick?.(post.user)}>
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-[var(--color-border)]/30 text-[var(--color-text-dim)] text-xs">
            ...
            <button onClick={handleSave} className={cn('ml-auto flex items-center gap-1.5', saved ? 'text-[#ff8d49]' : 'hover:text-[var(--color-text)]')}>
  ...
  return (
    <article className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-sm overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-6 pb-4">
        <Avatar className="h-12 w-12 cursor-pointer shrink-0 border-2 border-[#ff8d49]" onClick={() => onUserClick?.(post.user)}>
          <AvatarImage src={post.user.avatar} /><AvatarFallback>{post.user.name[0]}</AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {post.isFeatured && (
              <span className="px-2.5 py-0.5 bg-[#ff8d49] text-white text-xs font-medium rounded-full">Featured</span>
            )}
          </div>
        ...
        {isOwner && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-2 rounded-full hover:bg-[var(--color-bg)]"><MoreHorizontal className="h-5 w-5 text-[var(--color-text-dim)]" /></button>
            </DropdownMenuTrigger>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-[var(--color-border)]/30">
        <div className="flex items-center gap-6 text-[var(--color-text-dim)] text-sm">
          <button onClick={handleLike} className={cn('flex items-center gap-2', liked && 'text-[#ff8d49]')}>
        ...
        <button onClick={handleSave} className={cn('flex items-center gap-2', saved ? 'text-[#ff8d49]' : 'text-[var(--color-text-dim)] hover:text-[var(--color-text)]')}>
