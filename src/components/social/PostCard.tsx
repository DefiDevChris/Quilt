...
export function PostCard({
  ...
  if (viewMode === 'grid') {
    return (
      <div className="bg-white rounded-2xl border border-[#e8e1da] shadow-sm hover:shadow-md transition-shadow duration-200">
        <div className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-t-2xl"
          ...
          <div className="absolute top-2 right-2">
            <button onClick={(e) => { e.stopPropagation(); handleLike(); }}
              className={cn('p-2 rounded-full', liked ? 'bg-[#ff8d49] text-white' : 'bg-white/90 text-[#6b655e]')}>
  ...
  /* ── Full View ── */
  return (
    <div className="bg-white rounded-2xl border border-[#e8e1da] shadow-sm">
      {/* Header */}
        ...
        {isOwner && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-1.5 rounded-full hover:bg-gray-50"><MoreHorizontal className="h-5 w-5 text-[#6b655e]" /></button>
            </DropdownMenuTrigger>
