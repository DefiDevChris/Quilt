...
export function PatternAdjuster() {
  const { updatePatternTransform, getPatternInfo } = useFabricPattern();
  ...
  const handleChange = useCallback(
    (field: keyof PatternState, value: number) => {
  ...
  if (!state.hasPattern) return null;

  return (
    <div className="border-t border-outline-variant px-3 py-2">
      <h3 className="text-caption font-medium text-secondary uppercase tracking-wider mb-2">
        Pattern Fill
      </h3>
      ...
      <div className="space-y-2">
        <div>
          <label className="flex items-center justify-between text-xs text-secondary">
            <span>Scale X</span>
            <span className="text-caption text-secondary font-mono">{state.scaleX.toFixed(2)}</span>
          </label>
          <input
            type="range"
            min={10}
        ...
        <div>
          <label className="flex items-center justify-between text-xs text-secondary">
            <span>Scale Y</span>
            <span className="text-caption text-secondary font-mono">{state.scaleY.toFixed(2)}</span>
          </label>
          <input
            type="range"
            min={10}
        ...
        <button
          ...
          className="w-full rounded-md bg-background px-2 py-1 text-caption text-secondary hover:bg-outline-variant"
        >
