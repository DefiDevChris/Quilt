...
import { BLOCK_OVERLAYS, PATTERN_OVERLAYS, type BlockOverlay } from '@/lib/quilt-overlay-registry';
import { RecommendedDimensionsModal } from './RecommendedDimensionsModal';

type OverlayType = 'block' | 'pattern';
...
const DIFFICULTY_COLORS: Record<BlockOverlay['difficulty'], string> = {
...
export function BlockOverlaySelector({
  ...
  const filteredPatterns = PATTERN_OVERLAYS.filter(
    (p) =>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-on-surface/40">
      <div className="w-[700px] max-h-[80vh] rounded-xl bg-surface p-5 shadow-elevation-4 overflow-hidden flex flex-col">
        ...
        <div className="mb-3 flex gap-1 rounded-lg bg-background p-1">
          ...
          >
            Patterns ({PATTERN_OVERLAYS.length})
          </button>
        </div>
        ...
        <input
          ...
          placeholder="Search blocks or patterns..."
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              {filteredBlocks.map((block) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${DIFFICULTY_COLORS[block.difficulty]}`}
                    >
                      {block.difficulty}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {filteredPatterns.map((pattern) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setPendingOverlay({ path: pattern.svgPath, type: 'pattern' });
          ...
          {((activeType === 'block' && filteredBlocks.length === 0) ||
            (activeType === 'pattern' && filteredPatterns.length === 0)) && (
