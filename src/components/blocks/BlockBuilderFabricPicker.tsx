...
export function BlockBuilderFabricPicker({ onFabricDragStart }: BlockBuilderFabricPickerProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Search */}
      <div className="px-3 py-2 border-b border-[var(--color-border)]/15">
        <input
          type="text"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="Search fabrics..."
          className="w-full rounded-full border border-[var(--color-border)] bg-[var(--color-bg)] px-2.5 py-1.5 text-xs focus:border-primary focus:outline-none"
        />
      </div>
...
function FabricSwatch({
  ...
  return (
    <div
      ...
      className="group relative cursor-grab rounded-full border border-[var(--color-border)] overflow-hidden hover:border-primary transition-colors"
      title={fabric.name}
    >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-[var(--color-bg)] px-1 pb-0.5 pt-2">
        <p className="text-[9px] text-[var(--color-text)] truncate">{fabric.name}</p>
      </div>
    </div>
  );
}
