...
export function DimensionsStep() {
  ...
  const lockAspectRatio = usePhotoLayoutStore((s) => s.lockAspectRatio);
  const setTargetDimensions = usePhotoLayoutStore((s) => s.setTargetDimensions);
  const setSeamAllowance = usePhotoLayoutStore((s) => s.setSeamAllowance);
  const setLockAspectRatio = usePhotoLayoutStore((s) => s.setLockAspectRatio);
  ...
  return (
    <div className="flex flex-col gap-6 h-full overflow-y-auto">
      ...
      <div className="flex justify-end mt-auto flex-shrink-0">
        <button type="button" onClick={handleCalculatePieces} className="btn-primary-sm">
          Calculate Pieces
        </button>
      </div>
    </div>
