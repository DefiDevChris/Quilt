...
export default function OrderTimeline({ statusHistory }: OrderTimelineProps) {
  if (!statusHistory || statusHistory.length === 0) {
    return (
      <div className="text-sm text-[#4a4a4a] font-['Montserrat'] italic">
      ...
      {statusHistory.map((entry, index) => (
        <div key={index} className="flex gap-4">
          ...
          <div className="flex flex-col items-center">
            <div className="w-3 h-3 rounded-full bg-[#7CB9E8] flex-shrink-0" />
          ...
          <div className="pb-4">
            ...
            {entry.reason && (
              <p className="text-sm text-[#4a4a4a] font-['Montserrat'] mt-1">{entry.reason}</p>
            )}
            <p className="text-xs text-[#4a4a4a] font-['Montserrat'] mt-1">
