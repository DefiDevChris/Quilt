...
export function NewProjectWizard(props: NewProjectWizardInternalProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const defaultStudioCard =
        LAYOUT_TYPE_CARDS.find((card) => card.id === DEFAULT_STUDIO_LAYOUT_ID) ?? LAYOUT_TYPE_CARDS[0];
  ...
  const showStudioLayoutStep = step === 1;
  ...
  return (
    ...
    >
      <div ref={dialogRef} tabIndex={-1} className="flex-1 flex flex-col overflow-y-auto focus:outline-none relative">
        {/* Top navigation */}
            ...
            {showConfigStep && studioCard && (
              <div className="space-y-6 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 flex-shrink-0 rounded-lg bg-[var(--color-bg)] border border-[var(--color-border)]/15 flex items-center justify-center overflow-hidden p-1">
                      {PRESET_SVG[studioCard.defaultPresetId] ? (
                        <div className="w-full h-full" dangerouslySetInnerHTML={{ __html: PRESET_SVG[studioCard.defaultPresetId] }} />
                      ) : (
                  ...
                  <>
                    ...
                    <div className="rounded-lg border border-[var(--color-border)]/20 bg-[var(--color-bg)] px-4 py-3">
                      ...
                      <p className="mt-2 text-[13px] leading-[20px] text-[var(--color-text-dim)]">
                        Blocks can drop anywhere on the quilt grid and shapes stay aligned with snap-to-grid.
                      </p>
                    </div>
                  </>
                ) : (
                  <>
                    {studioCard.hasGridConfig && (
                      <div className="space-y-3 rounded-lg border border-[var(--color-border)]/20 p-4">
                        <WizardSliderRow label="Rows" value={studioRows} min={1} max={20} step={1} suffix={`${studioRows}`} onChange={setStudioRows} />
                        <WizardSliderRow label="Columns" value={studioCols} min={1} max={20} step={1} suffix={`${studioCols}`} onChange={setStudioCols} />
                        <WizardSliderRow label="Block size" value={studioBlockSize} min={2} max={24} step={0.5} suffix={`${studioBlockSize}″`} onChange={setStudioBlockSize} />
                      </div>
                    )}
                    ...
                    {studioCard.hasSashing && (
                      <div className="rounded-lg border border-[var(--color-border)]/20 p-4">
                        <WizardSliderRow
                          ...
                          onChange={(value) => setStudioSashing((current) => ({ ...current, width: value }))}
                        />
                      </div>
                    )}
                    ...
                    {studioCard.hasBorders && (
                      <div className="rounded-lg border border-[var(--color-border)]/20 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[14px] leading-[20px] font-medium text-[var(--color-text)]">Borders</span>
                            ...
                            <button
                              ...
                              onClick={() =>
                                setStudioBorders((current) => [
                                  ...current,
                                  { id: createBorderId(), width: 2, color: '#d4c4b5', fabricId: null, type: 'solid' },
                                ])
                              }
                            ...
                            {studioBorders.map((border, index) => (
                              <div key={border.id ?? index} className="rounded-lg bg-[var(--color-bg)] px-3 py-3">
                                <div className="mb-2 flex items-center justify-between gap-3">
                                  <span className="text-[13px] leading-[20px] font-medium text-[var(--color-text)]">Border {index + 1}</span>
                                  <button
                                    type="button"
                                    onClick={() => setStudioBorders((current) => current.filter((_, ci) => ci !== index))}
                                ...
                                <WizardSliderRow
                                  ...
                                  onChange={(value) =>
                                    setStudioBorders((current) =>
                                      current.map((cb, ci) => (ci === index ? { ...cb, width: value } : cb))
                                    )
                                  }
                                />
                              </div>
                    ...
                    {studioCard.hasBinding && (
                      <div className="rounded-lg border border-[var(--color-border)]/20 p-4">
                        <WizardSliderRow label="Binding" value={studioBindingWidth} min={0} max={2} step={0.125} suffix={`${studioBindingWidth}″`} onChange={setStudioBindingWidth} />
                      </div>
                    )}
                ...
                <div className="flex justify-center gap-3 mt-6 pb-4">
                  ...
                  >
                    {isStudio ? (isCreating ? 'Applying...' : 'Apply') : (isCreating ? 'Creating...' : 'Open Studio')}
                  </button>
                </div>
              </div>
            )}
...
function WizardSliderRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <label htmlFor={id} className="w-20 flex-shrink-0 text-[13px] leading-[20px] text-[var(--color-text-dim)]">
        {label}
      </label>
      <input
        id={id}
