...
export function PdfExportDialog({ isOpen, onClose }: PdfExportDialogProps) {
  ...
  return (
    ...
    >
      {/* Mode selector */}
      <div className="mb-4">
        <label className="mb-1 block text-xs font-medium text-[#1a1a1a]">Export Type</label>
        ...
        <p className="mt-1 text-[10px] text-[#4a4a4a]">{MODE_INFO[exportMode].description}</p>
      </div>

      {/* Printlist summary — for pattern-pieces and cut-list modes */}
      {needsPrintlist && (
        <div className="mb-4 rounded-lg border border-[#d4d4d4] bg-white p-3">
      ...
      {/* Paper size */}
      <div className="mb-4">
        <label className="mb-1 block text-xs font-medium text-[#1a1a1a]">Paper Size</label>
        <select
          value={paperSize}
          onChange={(e) => setPaperSize(e.target.value as PaperSize)}
          className="w-full rounded-lg border border-[#d4d4d4] bg-white px-3 py-2 text-sm text-[#1a1a1a]"
        >
