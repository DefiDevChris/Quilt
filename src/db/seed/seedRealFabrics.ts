...
const COLLECTIONS: CollectionDef[] = [
  {
    ...
    description:
      'A cozy winter-inspired collection featuring delicate berry motifs, soft tonal prints, and cottage-style florals. Perfect for holiday quilts, table runners, and traditional projects.',
    priceMin: 9.99,
    priceMax: 13.99,
  ...
  },
  {
    ...
    description:
      "Charisma Horton's Our Simple Life brings modern minimalism to quilting with clean geometrics, fresh colorways, and versatile prints. Ideal for contemporary quilts and modern home decor.",
    priceMin: 10.49,
    priceMax: 14.49,
  ...
  },
  {
    ...
    description:
      "Charisma Horton's Dream Weaver features dreamy abstract weaves, organic textures, and vibrant modern color palettes. A versatile collection for contemporary quilts, apparel, and modern projects.",
    priceMin: 10.49,
    priceMax: 14.49,
    skuPrefix: '10227',
  },
  {
    ...
    description:
      'Classic English countryside designs from Makower UK featuring beautiful florals (pansies, roses, daffodils, tulips), gingham checks, and garden-inspired prints. Perfect for traditional quilts and country-style projects.',
    priceMin: 11.99,
    priceMax: 15.99,
    skuPrefix: 'MU_',
  },
  {
    ...
    description:
      "Alison Glass's Color Camp Grove brings nature-inspired greens, earthy tones, and her signature modern aesthetic. Features leafy prints, organic textures, and fresh green palettes.",
    priceMin: 11.99,
    priceMax: 15.49,
  ...
  },
  {
    ...
    description:
      "Alison Glass's Color Camp Sun radiates with warm golden yellows, sunset oranges, and vibrant energy. Perfect for cheerful quilts, summer projects, and bold modern designs.",
    priceMin: 11.99,
    priceMax: 15.49,
  ...
  },
  {
    ...
    description:
      "Alison Glass's Color Camp Bloom features floral-inspired prints, fresh pinks, and rosy tones. A beautiful collection for feminine quilts, baby quilts, and garden-themed projects.",
    priceMin: 11.99,
    priceMax: 15.49,
...
const COLOR_CODE_MAP: Record<string, { color: string; family: string; hex: string }> = {
  ...
  LRS: { color: 'Light Rose', family: 'pink', hex: '#F8BBD0' },
...
function getCollectionForImage(filename: string): CollectionDef | null {
  ...
  if (baseName.startsWith('1042') || baseName.startsWith('1043')) {
    ...
    if (colorInfo) {
      if (colorInfo.family === 'green')
        return COLLECTIONS.find((c) => c.name === 'Color Camp - Grove')!;
      if (colorInfo.family === 'yellow' || colorInfo.family === 'orange')
        return COLLECTIONS.find((c) => c.name === 'Color Camp - Sun')!;
      if (colorInfo.family === 'pink' || colorInfo.family === 'red')
        return COLLECTIONS.find((c) => c.name === 'Color Camp - Bloom')!;
    }
    // Default to Grove
    return COLLECTIONS.find((c) => c.name === 'Color Camp - Grove')!;
  }
...
): { color: string; family: string; hex: string } {
  const baseName = filename.replace('.jpg', '');
...
async function main() {
  ...
  const products = images
    .map((filename, index) => {
      ...
      const price = parseFloat(
        (collection.priceMin + Math.random() * (collection.priceMax - collection.priceMin)).toFixed(
          2
        )
      );
  ...
  fs.writeFileSync(
    path.join(outputDir, 'real-fabric-products.json'),
