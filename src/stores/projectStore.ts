...
interface ProjectStoreState {
  ...
  /**
   * Set when a Template-mode project is committed via "Start Designing".
   * The template hydration hook reads this once the canvas is mounted,
   * stamps the template's pre-baked blocks + fabric assignments onto the
   * canvas, then clears it. Distinct from `mode === 'template'` because
   * that flag persists for the project's lifetime; this one is one-shot.
   */
