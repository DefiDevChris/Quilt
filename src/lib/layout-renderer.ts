 ...
 */
function categoryToLayoutType(
  category: LayoutCategory
): 'grid' | 'sashing' | 'on-point' {
  switch (category) {
    case 'straight':
      return 'grid';
    case 'sashing':
...
function convertResultToAreas(
  ...
  if (template.category === 'sashing') {
    const { sashingAreas, cornerstoneAreas } = separateSashingAndCornerstones(
      result,
      template
    );
...
function separateSashingAndCornerstones(
  ...
  for (const strip of result.sashingStrips) {
    const isSquare =
      Math.abs(strip.width - strip.height) < 0.01 &&
      strip.width > 0;
 ...
 */
function computeMedallionAreas(
  template: LayoutTemplate,
  pxPerUnit: number
): LayoutArea[] {
  const areas: LayoutArea[] = [];
  ...
  // Center block
  const totalBorderWidth = template.borders.reduce(
