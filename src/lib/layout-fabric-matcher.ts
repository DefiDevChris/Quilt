 ...
 *
 * Matches parsed layout fabrics to database fabric records using a
...
import type { ParsedFabric } from './layout-parser-types';
...
export interface FabricMatch {
  ...
  readonly layoutSku: string | null;
...
function trySkuMatch(
  layoutFabric: ParsedFabric,
  skuIndex: ReadonlyMap<string, FabricRecord>
): FabricRecord | null {
  if (!layoutFabric.sku) return null;

  const normalized = normalizeSku(layoutFabric.sku);
...
function tryNameMatch(
  ...
  const layoutName = normalizeName(layoutFabric.name);
  ...
  for (const dbFabric of dbFabrics) {
    ...
    if (dbFabric.collection) {
      ...
      const dist = levenshteinDistance(layoutName, collectionName);
    ...
    const dist = levenshteinDistance(layoutName, dbName);
    if (dist < bestDistance) {
      bestDistance = dist;
      bestMatch = dbFabric;
    }
...
function tryColorFamilyMatch(
  ...
  const targetFamily = layoutFabric.colorFamily ?? inferColorFamily(layoutFabric.name);
...
/**
 * Match an array of parsed layout fabrics to database fabric records.
 *
 * For each layout fabric, tries three strategies in order:
...
export function matchPatternFabrics(
  layoutFabrics: readonly ParsedFabric[],
  dbFabrics: readonly FabricRecord[]
): FabricMatch[] {
  const skuIndex = buildSkuIndex(dbFabrics);

  return layoutFabrics.map((layoutFabric): FabricMatch => {
    // Strategy 1: Exact SKU match
    const skuMatch = trySkuMatch(layoutFabric, skuIndex);
    if (skuMatch) {
      return {
        layoutLabel: layoutFabric.label,
        layoutName: layoutFabric.name,
        layoutSku: layoutFabric.sku ?? null,
    ...
    const nameMatch = tryNameMatch(layoutFabric, dbFabrics);
    if (nameMatch) {
    ...
    const colorMatch = tryColorFamilyMatch(layoutFabric, dbFabrics);
    if (colorMatch) {
      return {
        layoutLabel: layoutFabric.label,
        layoutName: layoutFabric.name,
    ...
    return {
      ...
      confidence: 0,
      colorHex: inferColorHex(layoutFabric.name, layoutFabric.colorFamily ?? undefined),
      matchMethod: 'none',
    };
  });
}
