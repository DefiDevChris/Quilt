...
import type { ParsedPattern, ParsedBlock, ParsedLayout } from './layout-parser-types';
...
export function buildOnPointBlockObjects(
  ...
  const bgFabric = fabricMatches.find((fm) => fm.layoutLabel.toLowerCase().includes('background'));
...
export function resolveBorderColors(
  ...
  const borderFabrics = fabricMatches.filter((fm) =>
    fm.layoutLabel.toLowerCase().includes('border')
  );
...
export function resolveCornerstoneColor(fabricMatches: readonly FabricMatch[]): string {
  const accentFabric = fabricMatches.find((fm) => fm.layoutLabel.toLowerCase().includes('accent'));
