...
- Name: projects.spec.ts >> Profile & Settings >> settings page loads
- Location: tests/e2e/projects.spec.ts:319:7
...
Locator: getByText(/settings|profile|account/i)
Expected: visible
Error: strict mode violation: getByText(/settings|profile|account/i) resolved to 8 elements:
    1) <p class="text-dim text-[14px] leading-[20px] font-normal">Account</p> aka getByText('Account', { exact: true })
    2) <h1 class="text-default text-[40px] leading-[52px] font-normal">Settings</h1> aka getByRole('heading', { name: 'Settings' })
    3) <p class="text-dim text-[18px] leading-[28px] font-normal">Manage your profile, billing, and account.</p> aka getByText('Manage your profile, billing')
    4) <p class="text-[14px] leading-[20px] mb-1">Account Settings</p> aka getByText('Account Settings')
    5) <h2 class="text-[24px] leading-[32px] text-[var(--color-text)]">Delete Account</h2> aka getByRole('heading', { name: 'Delete Account' })
    6) <h3 class="text-[18px] leading-[28px] text-[var(--color-text)]">Request Account Deletion</h3> aka getByRole('heading', { name: 'Request Account Deletion' })
    7) <p class="text-[16px] leading-[24px] text-[var(--color-text-dim)] max-w-xl">This will permanently remove all your projects, f…</p> aka getByText('This will permanently remove')
    8) <button class="group flex items-center gap-4 px-8 py-4 text-[var(--color-text)] rounded-full text-[16px] leading-[24px]">…</button> aka getByRole('button', { name: 'Request Account Deletion' })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText(/settings|profile|account/i)
...
```ts
  221 |   });
  222 | 
  223 |   test('project duplication works', async ({ page }) => {
  224 |     await page.goto('/projects');
  225 |     const actionMenu = page.locator('[aria-label*="actions"], button:has(svg)').first();
  226 |     if (await actionMenu.isVisible()) {
  227 |       await actionMenu.click();
  228 |       const duplicateButton = page.getByRole('menuitem', { name: /duplicate|copy/i }).or(
  229 |         page.getByText(/duplicate|copy/i)
  230 |       );
  231 |       if (await duplicateButton.isVisible()) {
  232 |         await duplicateButton.click();
  233 |         await expect(page.getByText(/duplicate.*created|copy.*created|success/i)).toBeVisible();
  234 |       }
  235 |     }
  236 |   });
  237 | 
