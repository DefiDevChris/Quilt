...
- Name: onboarding.spec.ts >> Help Panel >> help button is visible in studio
- Location: tests/e2e/onboarding.spec.ts:75:7
...
Locator: getByRole('button', { name: /help|\?/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /help|\?/i }).first()
...
```ts
  ...
  3   | 
  4   | test.describe('Onboarding Tour', () => {
  5   |   test.beforeEach(async ({ page }) => {
  6   |     await mockAuth(page, 'pro');
  7   |     await mockCanvas(page);
  8   |     await mockProject(page, 'test-project-1');
  9   |     await page.evaluate(() => {
  10  |       localStorage.removeItem('quiltcorgi-onboarding-completed');
  11  |       localStorage.removeItem('onboardingCompleted');
  12  |     });
  13  |   });
  14  | 
