...
- Name: blog.spec.ts >> Blog Section >> blog post has publish date
- Location: tests/e2e/blog.spec.ts:76:7
...
Locator: locator('text=\\d{4}').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=\\d{4}').first()
