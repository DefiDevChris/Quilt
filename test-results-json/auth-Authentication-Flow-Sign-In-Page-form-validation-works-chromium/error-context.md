...
- Name: auth.spec.ts >> Authentication Flow >> Sign In Page >> form validation works
- Location: tests/e2e/auth.spec.ts:37:9
...
Locator: getByText(/email.*required|required/i).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/email.*required|required/i).first()
