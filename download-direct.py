...
# Block definitions: (num, name, scissortail_image_name)
# The image name is based on what Scissortail uses in their wp-content/uploads
blocks = [
...
for num, name, img_name in blocks:
    ...
    # Try direct Scissortail image URL
    url = f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{img_name}.png"
    size = download_image(url, filepath)
    
    if size > 2000:
        print(f"{num:3d}/105: {name:30s} OK ({size}B)")
        downloaded += 1
    else:
        # Try alternate URL patterns
        alt_urls = [
            f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{img_name}.jpg",
            f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{img_name.replace('-', '_')}.png",
            f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{img_name.replace('-', '_')}.jpg",
            f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{name.replace(' ', '-')}.png",
            f"https://scissortailquilting.com/wp-content/uploads/2017/09/BlockLib_{name.replace(' ', '_')}.png",
        ]
        success = False
        for alt_url in alt_urls:
            size = download_image(alt_url, filepath)
            if size > 2000:
                print(f"{num:3d}/105: {name:30s} OK-ALT ({size}B)")
                downloaded += 1
                success = True
                break
        if not success:
