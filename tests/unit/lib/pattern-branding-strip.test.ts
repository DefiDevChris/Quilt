...
import {
  ...
  stripPatternName,
  stripDesignerName,
} from '@/lib/pattern-branding-strip';

describe('pattern-branding-strip', () => {
  ...
  describe('stripPatternName', () => {
    it('strips collection attribution via " - " separator', () => {
      expect(
        stripPatternName(
          'Winter Jewels Quilt - Sugarberry by Andover Fabrics'
        )
      ).toBe('Winter Jewels Quilt');
    });

    it('strips collection via " - " when no manufacturer follows', () => {
      expect(
        stripPatternName('Turn of the Century Quilt - Century Solids')
      ).toBe('Turn of the Century Quilt');
    });

    it('strips collection with year suffix', () => {
      expect(stripPatternName('Crossover Quilt - Sun Print 2025')).toBe(
        'Crossover Quilt'
      );
    });

    it('returns the name unchanged when no separator is present', () => {
      expect(stripPatternName('Simple Quilt')).toBe('Simple Quilt');
    });
  });
