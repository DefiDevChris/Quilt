...
test.describe('Community Post Right-Click Menu', () => {
  test.beforeEach(async ({ page }) => {
    await mockAuth(page, 'pro');
  ...
  test('right-click on post shows menu', async ({ page }) => {
    const post = page.locator('[data-testid="community-post"]').first();
  ...
  test('post context menu - share option', async ({ page }) => {
    const post = page.locator('[data-testid="community-post"]').first();
    if (await post.isVisible()) {
      await post.click({ button: 'right' });
      await page.waitForTimeout(500);
  ...
  test('post context menu - report option', async ({ page }) => {
    const post = page.locator('[data-testid="community-post"]').first();
    if (await post.isVisible()) {
      await post.click({ button: 'right' });
      await page.waitForTimeout(500);
