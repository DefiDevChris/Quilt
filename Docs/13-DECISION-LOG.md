...
| 21 | Tech | Auth service | AWS Cognito (email/password + future OAuth federation) | Built-in email verification and password reset flows. JWT-based stateless sessions verified via JWKS. Keeps auth in the AWS ecosystem with Secrets Manager for credential storage. Migrated from NextAuth.js (March 2026). | NextAuth.js (original choice), Firebase Auth, Supabase Auth |
...
| 43 | Security | Token strategy | JWT (RS256, Cognito-signed). ID/access tokens: 1hr. Refresh token: 30 days in HTTP-only cookie. | Stateless verification via JWKS. Automatic token refresh in `getSession()`. No server-side session store. | Session-based, Short-lived JWT without refresh |
| 44 | Security | Password hashing | Cognito-managed (PBKDF2, configurable iteration count) | Delegated to Cognito — no password handling in application code. | bcrypt (previous), argon2, scrypt |
| 47 | Security | Secret management | AWS Secrets Manager (KMS-encrypted) | All production credentials in one KMS-encrypted secret (`quiltcorgi/prod`). Loaded at startup via `instrumentation.ts`. No secrets in .env files in production. `AWS_SECRET_NAME=skip` for local dev. | .env files (previous), AWS SSM Parameter Store, HashiCorp Vault |
